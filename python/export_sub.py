...
class Subcribe():
    ...
    """
    def __init__(self, app_client_id, app_client_secret, verbose=True,**kwargs):
        """
    ...
    def on_new_eeg_data(self, *args, **kwargs):
        ...
        """
        data = kwargs.get('data')
        self.data['eeg'].loc[len(self.data['eeg'])] = data['eeg']
        if self.verbose:
    ...
    def on_new_mot_data(self, *args, **kwargs):
        ...
        """
        data = kwargs.get('data')
        self.data['mot'].loc[len(self.data['mot'])] = data['mot']
        if self.verbose:
    ...
    def on_new_dev_data(self, *args, **kwargs):
        ...
        """
        data = kwargs.get('data')
        self.data['dev'].loc[len(self.data['dev'])] = data['dev']
        if self.verbose:
    ...
    def on_new_met_data(self, *args, **kwargs):
        ...
        """
        data = kwargs.get('data')
        self.data['met'].loc[len(self.data['met'])] = data['met']
        if self.verbose:
    ...
    def on_new_pow_data(self, *args, **kwargs):
        ...
        """
        data = kwargs.get('data')
        self.data['pow'].loc[len(self.data['pow'])] = data['pow']
        if self.verbose:
...
def main():
    ...
    # your_app_client_id = 'KD7TAk1pdrSLO42V3XrYJhxv6d15zgmyRsVwRT98'
    # your_app_client_secret = 'vwBoZJiw0fWgKHMweEBRRDkRjGcfk9TK0X95lUcgOuEd2enAqKnYt9T9nsfdaDOtUwd5nie1XFw1wjuQtF7HzATBRbB09L35yHcHRWhAVAFd318ZNrtN7yfEU4sjm6nX'
    ...
    # test-train-eeg
    # your_app_client_id='RwWirsg42pWZmc1KBADxoLKsChMIEmxegtjtlt32'
    # your_app_client_secret='7vTjKJAGveg6QPjfU2iUoGZrFY22XMNnpfi4vcybK69dVJriucme9bv4tfmKo9fFHAiCJNgnYlpr54ZHUR6JTD2yObEmV4qTtWvsdnsRYTsb7h4UeSEU6zytumJEuAjc'

    # test-train
    # your_app_client_id = "xLmmGL8kOq0tjihn8MlztMkXuMR2WF24cMwpcUjx"
    # your_app_client_secret = "0ZmTJF6R6HxqvGvT9JOUuR0sB6h7NrztSXrHgJn9hxvOstAxkz8zAMyguoBmZ8h8LASrstiDWDqlU5LPBKjQo3bS0dpvGsu66wDhKNiUJz65dwCxBRqnEXGieOQHNXb7"

    # test-train-Max
    your_app_client_id = "mhEpYaeTcU8QbGGsix0eLHO7aDxnPg6mQ6H0c7Q0"
    your_app_client_secret = "tToy7XWouBUoqJTKSe0OZFFcaUjVG0ozyQ4vdlRUGX8fVVHbY5L6j6MBiMa1IoIvFLZnQ3CrudbhPRulwrf6bSoJu3endW13uwEh8aP3we6MyukhvLY5p3plIywV7RKb"

    s = Subcribe(your_app_client_id, your_app_client_secret, verbose=False)

    # name = input("Nombre de la persona: ")
    name = "virtual"
    # t0 = int(input("Tiempo (en s): "))
    t0 = 60
    ...
    t = threading.Thread(target=s.start, args=(streams,))
    t.start()
    ...
    # s.join()
    time.sleep(3)
    # if os.path.exists(f"sub_data/{name}"):
    #     name = name + "_" + str(int(time.time()))
    # for stream in s.data:
    #     s.data[stream].to_csv(f'data_example/{name}_{stream}.csv')


    s.c.ws.keep_running = False
    t.join()

    if os.path.exists(f"sub_data/{name}"):
        # name = name + "_" + str(int(time.time())) # to avoid overwriting
        shutil.rmtree(f"sub_data/{name}") # remove existing directory if it exists
        
    os.mkdir(f"sub_data/{name}")

    for stream in s.data:
        s.data[stream].to_csv(f"sub_data/{name}/{stream}.csv")
